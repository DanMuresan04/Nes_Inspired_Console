...
##Switches
#set_property -dict { PACKAGE_PIN J15   IOSTANDARD LVCMOS33 } [get_ports { reset_rtl_0 }]; #IO_L24N_T3_RS0_15 Sch=sw[0]
...
## LEDs
#set_property -dict { PACKAGE_PIN H17   IOSTANDARD LVCMOS33 } [get_ports { DDR2_calib_complete }]; #IO_L18P_T2_A24_15 Sch=led[0]
...
set_property -dict { PACKAGE_PIN V14   IOSTANDARD LVCMOS33 } [get_ports { LED_ARP_IP }]; #IO_L22N_T3_A04_D20_14 Sch=led[13]
set_property -dict { PACKAGE_PIN V12   IOSTANDARD LVCMOS33 } [get_ports { LED_ARP_MAC }]; #IO_L20N_T3_A07_D23_14 Sch=led[14]
set_property -dict { PACKAGE_PIN V11   IOSTANDARD LVCMOS33 } [get_ports { LED_ARP_READY }]; #IO_L21N_T3_DQS_A06_D22_14 Sch=led[15]
...
##Micro SD Connector (PMOD E / D) mapped to custom AXI Quad SPI and AXI GPIO
set_property -dict { PACKAGE_PIN E2    IOSTANDARD LVCMOS33 } [get_ports { SD_RESET }]; 

# SPI Clock (SCK)
set_property -dict { PACKAGE_PIN B1    IOSTANDARD LVCMOS33 } [get_ports { spi_rtl_sck_io }]; 

# SPI MOSI (Nexys A7 SD_CMD pin)
set_property -dict { PACKAGE_PIN C1    IOSTANDARD LVCMOS33 } [get_ports { spi_rtl_io0_io }]; 

# SPI MISO (Nexys A7 SD_DAT[0] pin)
set_property -dict { PACKAGE_PIN C2    IOSTANDARD LVCMOS33 } [get_ports { spi_rtl_io1_io }]; 

# SPI Chip Select / Slave Select (Nexys A7 SD_DAT[3] pin)
set_property -dict { PACKAGE_PIN D2    IOSTANDARD LVCMOS33 } [get_ports { spi_rtl_ss_io[0] }];

#dummy (unused but keeping pins tied safely if needed)
#set_property -dict { PACKAGE_PIN J18   IOSTANDARD LVCMOS33 } [get_ports { SD_CARD_pin8_io }]; 
#set_property -dict { PACKAGE_PIN K15   IOSTANDARD LVCMOS33 } [get_ports { SD_CARD_pin9_io }]; 
#set_property -dict { PACKAGE_PIN J15   IOSTANDARD LVCMOS33 } [get_ports { SD_CARD_pin10_io }];
...
#set_clock_groups -asynchronous -group [get_clocks sys_clk_pin] -group [get_clocks clk_out1_pixel_clock]

##Synchronization Register Placement Constraints
#set_property ASYNC_REG TRUE [get_cells -hierarchical *key_reg_sync_1_reg*]
#set_property ASYNC_REG TRUE [get_cells -hierarchical *key_reg_sync_2_reg*]

# In your .xdc file - relax CDC paths from AXI regs to sprite renderer inputs
set_max_delay -datapath_only -from [get_clocks clk_out1*] -to [get_clocks clk_out2*] 10.0
set_min_delay 0 -from [get_clocks clk_out1*] -to [get_clocks clk_out2*]

## Bypass DRCs for unconstrained GPIO bits (since AXI GPIO is 32-bit but only bit 0 is used)
